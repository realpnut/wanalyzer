...
def generate_report(text):
	...
	report=[]
	report.append("="*60)
	report.append("WANALYZER REPORT")
	report.append("="*60)
	report.append("")
	report.append(f"TOTAL LOGS: {len(lines)}")
	report.append("")
	...
	for l in lines:
		if l.startswith("IP:"):
			...
			report.append(f"> {l}")
		elif l.startswith("ISP:"):
			report.append(f"> {l}")
		elif l.startswith("WHOIS"):
			report.append(f"> {l}")
		elif l.startswith("CRAWLED:"):
			report.append(f"> {l}")

	report.append("")
	report.append("RAW DUMP")
	report.append("-"*60)
	report.extend(lines)
...
class App(ctk.CTk):
	def __init__(self):
		...
		self.title_label=ctk.CTkLabel(self,text="WANALYZER",font=("Consolas",26),text_color="#00ff88")
		...
		self.current_label=ctk.CTkLabel(self,text="NOW: -",text_color="#00ff88")
		...
		self.stop_btn=ctk.CTkButton(self,text="STOP",fg_color="red",command=stop_and_restart)
	...
	def update_ui(self):
		...
		self.current_label.configure(text=f"NOW: {current_url}")
